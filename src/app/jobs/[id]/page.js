...
export default function JobCard({ job, userType }) {
  ...
  const formatDeadline = (job) => {
    // Try both date and deadline fields with safe fallbacks
    const dateValue = job?.date || job?.deadline
    
    if (!dateValue) return t("jobCard.flexible") || "Flexible"
    
    try {
      const date = new Date(dateValue)
      if (isNaN(date.getTime())) {
        return t("jobCard.flexible") || "Flexible"
      }
      ...
      <CardContent className="pt-6">
        ...
        <div className="space-y-2 text-sm text-gray-500 dark:text-gray-400">
          ...
          <div className="flex items-center">
            ...
            <span>Preferred by: {formatDeadline(job)}</span>
          </div>
